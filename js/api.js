...
function buildCandidateOrigins() {
  ...
  // If you're already on port 3000, current origin should be enough.
  // If you're on 5500, then try backend on 3000 next.
...
export async function getBackendOrigin(forceRefresh = false) {
  ...
  backendOriginPromise = (async function () {
    ...
    throw new Error("No working backend found. Start the Express server on the same origin, or on port 3000 if you're using port 5500 for the HTML page.");
  })();
